import type { DefaultIconSvgOptions, FallbackSvgOptions } from "./types";
...
/** Raw props needed to render an initials badge, independent of any FallbackSvgOptions plumbing */
export interface InitialsSvgMarkupProps {
  text: string;
  size: number;
...
/**
 * Default SVG markup builder for the initials badge. Exposed separately so
 * createBankLogos({ renderInitialsSvg }) can swap this out while reusing the
 * same initials/color computation in generateInitialsSvg.
 */
export function defaultInitialsSvgMarkup(props: InitialsSvgMarkupProps): string {
...
export function generateInitialsSvg(
  ...
  const backgroundColor = options?.backgroundColor ?? getBrandColor(nameOrInitials);
  const textColor = options?.textColor ?? "#FFFFFF";

  const svgContent = defaultInitialsSvgMarkup({ text, size, backgroundColor, textColor });
